import os, warnings
os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE" # allow 2 runtimes to coexist (workaround)
os.environ["MKL_SERVICE_FORCE_INTEL"] = "1" # force Intel OMP if MKL appears
os.environ["OMP_NUM_THREADS"] = "1" # avoid oversubscription (optional)
...
.rollouts(num_rollout_workers=0) # avoid remote workers 
.build())

# Train several short iterations
for i in range(2): 
res = algo.train() 
